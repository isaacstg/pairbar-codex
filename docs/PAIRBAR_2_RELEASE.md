...
The known remote `main` at planning time was `4399fd2090d5e5aa152ec5f0aa0188f5f3e5c5cf`. Recheck it read-only before proposing publication or integration. A source recovery tag does not roll back migrated user data. Do not install an old binary over schema 3 or restore metadata automatically.
...
- [ ] Preview uses fake metadata/runtime only and cannot discover or control live accounts, register global shortcuts or change login items. The in-memory no-op test passed; a native preview observation remains pending.
...
Current adversarial verdict: **BLOCK** for public release. The integrated worktree passes the source-policy audit, 128 synthetic tests, Pairbar 2.0.0 (17) development build, ZIP integrity/checksum and strict all-architectures verification of Pairbar's ad-hoc signed bundle. The final adversarial Claude lifecycle/archive finding is fixed and covered by denial tests. It is not a frozen release commit. Safe-environment signed-in lifecycle, native visual/accessibility, exact-revision macOS CI, injected archive durability-failure coverage, Developer ID signing and notarization remain pending. Managed Claude is separately blocked by its signed-in Chat, Code and Cowork evidence gate.

Truthful development-delivery wording: “Dynamic profile implementation and 128 local synthetic checks are available in this branch as an ad-hoc development artifact. Managed Claude remains blocked pending signed-in isolation acceptance. Final live acceptance, Developer ID signing and notarization are not claimed.”
